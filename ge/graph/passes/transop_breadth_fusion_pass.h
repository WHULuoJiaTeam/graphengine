...
class TransOpBreadthFusionPass : public GraphPass {
 ...
 private:
  std::string GetNodeId(const int anchor_index, const NodePtr& node);

  std::map<std::string, std::vector<NodePtr>> GetOutputTransOpNodes(const NodePtr& node);

  graphStatus Fusion(const std::vector<NodePtr>& trans_nodes, ComputeGraphPtr& graph);

  std::string JoinDims(const std::string& sp, const std::vector<int64_t>& dims);
};
}  // namespace ge
