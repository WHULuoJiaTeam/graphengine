...
build_graphengine()
{
  ...
  if [[ "X$LUOJIANET_MS_MODE" = "Xoff" ]]; then
...
if [[ "X$ENABLE_GE_UT" = "Xoff" && "X$ENABLE_GE_ST" = "Xoff" && "X$LUOJIANET_MS_MODE" = "Xoff" ]]; then
  generate_package
elif [ "X$LUOJIANET_MS_MODE" = "Xon" ]
then
  cd "${OUTPUT_PATH}"
